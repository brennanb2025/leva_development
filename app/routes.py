...
def admin_login_post():
    ...
    if str(app.config['ADMIN_PASSWORD']) == request.form.get("password") and str(app.config['ADMIN_USERNAME']) == request.form.get("username"):
        session["userID"] = request.form.get(
            "username")  # put the username in the session
...
def admin_user_matches():  # mentees true = search for mentees, false = mentors
    ...
    for s_u in match_and_users:
        ...
        dictMenteeToMentor[s_u.User].append(
            User.query.filter_by(id=s_u.Select.mentor_id).first())
...
def progress():
    ...
    # the mentor or mentee that the user logged in has selected, or None
    select_mentor_mentee = None
    if isMentee:
        # the entry of the mentor-mentee selection, or None
        selectEntry = Select.query.filter_by(mentee_id=user.id).first()
        if selectEntry != None:
            select_mentor_mentee = User.query.filter_by(
                id=selectEntry.mentor_id).first()
            currentMeetingNumber = selectEntry.current_meeting_number_mentee
    else:
        selectEntry = Select.query.filter_by(mentor_id=user.id).first()
        if selectEntry != None:
            select_mentor_mentee = User.query.filter_by(
                id=selectEntry.mentee_id).first()
    ...
    if selectEntry != None and currentMeetingNumber != -1:
        ...
        for m in previousMeetings:  # build the dicts of the info about each meeting
            prevMeetingInfo.append(getCompletedMeetingInfo(
                m, isMentee, selectEntry.id, m.num_meeting))
    ...
    if isMentee:
        ...
        if selectEntry != None:
            ...
            if completionInfoMentee == None:
                ...
                completionInfoMentee.set_meeting_notes(
                    form.get("meetingNotes"), "mentee")
                completionInfoMentee.set_completion_timestamp(
                    "mentee")  # update timestamp

            selectEntry.inc_current_meeting_ID(
                "mentee")  # increment the meeting number
            db.session.commit()
        ...
        if selectEntry != None:
            ...
            if completionInfoMentor == None:
                ...
                completionInfoMentor.set_meeting_notes(
                    form.get("meetingNotes"), "mentor")
                completionInfoMentor.set_completion_timestamp(
                    "mentor")  # update timestamp

            selectEntry.inc_current_meeting_ID(
                "mentor")  # increment the meeting number
            db.session.commit()
...
def sign_inPost():
    ...
    if success:  # they entered an email and password - now check them
        ...
        if User.query.filter_by(email=form1.get('email')).first() == None:
            success = False
            flash(u'User does not exist. If you are a new user, click the "Make an Account" button below.', 'emailError')
        # email and password do not match
        elif not User.query.filter_by(email=form1.get('email')).first().check_password(form1.get('password')):
...
def get_popular_tags():
    tags = Tag.query.order_by(Tag.num_use.desc()).limit(
        500).all()  # sort by num_use and limit to 200
    carInts = CareerInterest.query.order_by(
        CareerInterest.num_use.desc()).limit(500).all()
...
def registerValidate2():
    ...
    if business == '':
        ...
        businessRegisteredUnder = Business.query.filter_by(
            name=business).first()
...
def registerPost():
    ...
    if form1.get('radio_contact') == 'Phone number' and form1.get('phoneNumber') == "":
    ...
    # if gender preference should be taken into account, check it.
    if str(app.config['MATCHING_FLAG_MENTOR_GENDER_PREFERENCE']) == "True":
        # mentee and mentor preference empty
        if isMentee and form1.get("radio_gender_preference") == None:
            flash(u"Please enter a preference for your mentor's gender.",
                  'mentor_preference_error')
            success = False
        # mentor and gender identity not entered
        if not isMentee and form1.get("radio_gender_identity") == None:
            flash(u"Please enter your gender identity.", 'gender_identity_error')
            success = False

    # if division preference should be taken into account, check it.
    if str(app.config['MATCHING_FLAG_DIVISION_PREFERENCE']) == "True":
        ...
        if form1.get("divisionPreference") == None:
            flash(u"Please enter a preference for your division.",
                  'division_preference_error')
            success = False

    # if personality should be taken into account, check it.
    if str(app.config['MATCHING_FLAG_PERSONALITY']) == "True":
        if form1.get('personality1') != None and form1.get('personality2') != None and form1.get('personality3') != None:
            if form1.get('personality1').strip() == "" or form1.get('personality2').strip() == "" or form1.get('personality3').strip() == "":
                flash(
                    u"Please enter three words or phrases that describe you.", 'personality_error')
                success = False
        else:
            flash(u"Please enter three words or phrases that describe you.",
                  'personality_error')
            success = False
    ...
    if form1.get('business') == '' or form1.get('business') == None:
        ...
        if businessRegisteredUnder == None:  # business doesn't exist in database
            ...
            if businessRegisteredUnder.number_employees_maximum == businessRegisteredUnder.number_employees_currently_registered:
                # every spot is taken in this business.
                success = False
                flash(u'That business has no spots left for more users.',
                      'businessError')
    ...
    if "resume" in request.files and request.files["resume"]:
        ...
        if resumeSize == -1:
            ...
            if file_ext == None:
                ...
                flash(u'Accepted file type: .pdf. You uploaded a',
                      file_ext + ".", 'resumeError')
                success = False
    ...
    if "croppedImgFile" in request.files and request.files.get("croppedImgFile").filename != '':
        ...
        if imgType == None:
            ...
            flash(u'Accepted file types: .png, .jpg. You uploaded a ' +
                  imgType + ".", 'imageError')
            success = False
    #image is optional
    """else:
    ...
    if success:  # success, registering new user
        ...
        if not isMentee or str(app.config['MATCHING_FLAG_MENTOR_GENDER_PREFERENCE']) == "False":
            # if mentor OR gender should not be taken into account, this should not be entered.
            mentor_gender_preferenceForm = None
        ...
        if isMentee or str(app.config['MATCHING_FLAG_MENTOR_GENDER_PREFERENCE']) == "False":
            # if mentee, this should not be entered.
            gender_identityForm = None
        ...
        businessRegisteredUnder = Business.query.filter_by(
            name=form1.get('business')).first()
        # increment number of users registered for this user
        businessRegisteredUnder.inc_number_employees_currently_registered()
        ...
        for i in range(int(form1.get('num_tags'))):
            ...
            if name != "":
                ...
                # I have to do this before I set the resource so the id will be set.
                db.session.commit()
        ...
        if "croppedImgFile" in request.files:
            ...
            if img:
                ...
                # set the user profile picture link
                user.set_profile_picture(output, filename)
                db.session.commit()
        ...
        if "resume" in request.files:
            resume = request.files["resume"]
            if resume:
                # I read the file before to check the length so I must put the cursor at the beginning in order to upload it.
                resume.seek(0)
    ...
    if form1.get('num_pairings') == '':
        ...
        if form1.get('num_pairings') == '0':
            ...
            try:
                ...
                success = False
                flash(
                    u'The number of mentors/mentees you are willing to have must be an integer.')
                errors.append("num_pairings")
...
def editProfile():
    ...
    # get the correct profile by inputting user id
    user = User.query.filter_by(id=session.get('userID')).first()
...
def editProfilePost():
    ...
    # changed current occupation --> check it
    if form.get("current_occupation") != u.current_occupation:
    ...
    if u.is_student and form.get("radio_gender_preference") != None:
        ...
        # changed preference --> check it
        if form.get("radio_gender_preference") != u.mentor_gender_preference:
    ...
    if not u.is_student and form.get("radio_gender_identity") != None:
        # changed gender identity --> check it
        if form.get("radio_gender_identity") != u.gender_identity:
    ...
    if form.get("divisionPreference") != u.division_preference:  # changed --> check it
        changedDivisionPreferenceSuccess = checkDivisionPreference(
            form, u.is_student)
    ...
    if success:
        ...
        # log data edit profile success
        logData(7, json.dumps(dataChangedDict))
...
def checkAttributes(form, isMentee):
    ...
    if int(form.get('num_career_interests')) == 0:
        if isMentee:
            flash(u'Please enter at least one career interest.',
                  'careerInterestError')
        else:
            flash(u'Please enter at least one career experience.',
                  'careerInterestError')
        return False
    return True
...
def checkPersonality(form):
    if form.get('personality1') != None and form.get('personality2') != None and form.get('personality3') != None:
        if form.get('personality1').strip() == "" or form.get('personality2').strip() == "" or form.get('personality3').strip() == "":
            flash(u'You must input 3 personality traits/phrases.',
                  'personalityError')
            return False
    else:
...
def checkDivisionPreference(form, isMentee):
    if form.get("divisionPreference") == None and form.get("divisionPreference") != "":
        # mentee and mentor division preference empty or mentor and mentee division preference empty
        if isMentee:
            flash(u"Please enter a preference for your mentor's division.",
                  'divisionPreferenceError')
        else:
            flash(u"Please enter a preference for your mentee's division.",
                  'divisionPreferenceError')
        return False
    return True
...
def checkContactPreference(form):
    # user chose to be contacted by phone
    if form.get('radio_contact') == 'Phone number' and form.get('phoneNumber') == "":
        flash(u'Your phone number cannot be empty.', 'phoneError')
        return False

    # somehow removed the check
    if form.get('radio_contact') != 'Email' and form.get('radio_contact') != 'Phone number':
        return False
...
def editProfilePassword():
    ...
    if success:
        User.query.filter_by(id=session['userID']).first().set_password(
            form.get('password'))  # set hashed pwd
        db.session.commit()
        return redirect(url_for('view', id=session.get('userID')))
    else:
        return redirect(url_for('editProfile'))
...
def editProfResume():
    ...
    if success:
        if resume:
            ...
            output, filename = upload_resume_file_to_s3(resume, user)
            # set the user profile picture link
            user.set_resume(output, filename)
            db.session.commit()
        else:
            success = False
...
def create_resume_link(user):
    if user.resume == None or user.resume_key == None:
        ...
        resumeUrl = s3_client.generate_presigned_url(
            'get_object',
            Params={'Bucket': str(
                app.config['BUCKET_NAME_RESUME']), 'Key': user.resume_key},
            ExpiresIn=100
        )
    return resumeUrl
...
def deleteProfile():
    ...
    # check input name = first name
    if user.first_name == form.get('first_name'):
        ...
        Business.query.filter_by(id=user.business_id).first(
        ).dec_number_employees_currently_registered()
...
def upload_media_file_to_s3(file_upload, user):
    ...
    output = 'https://{}.s3.amazonaws.com/{}'.format(
        app.config['BUCKET_NAME'], filename)
...
def upload_resume_file_to_s3(file_upload, user):
    ...
    output = 'https://s3-{}.amazonaws.com/{}/{}'.format(
        app.config['S3_REGION'], app.config['BUCKET_NAME_RESUME'], filename)
    print(filename, output, file_upload.content_type)
    db.session.commit()  # just in case
...
def delete_profile_picture(user):
    if user.profile_picture is not None:
        s3_client.delete_object(Bucket=str(
            app.config["BUCKET_NAME"]), Key=user.profile_picture_key)
...
def delete_intro_video(user):
    if user.intro_video is not None:
        s3_client.delete_object(Bucket=str(
            app.config["BUCKET_NAME"]), Key=user.intro_video_key)
...
def feedMentee(user):
    ...
    potentialUsers = User.query.filter_by(
        business_id=user.business_id).filter_by(is_student=False).all()

    users = []
    for u in potentialUsers:
        # only select users that have not already been chosen.
        if not mentorSelected(u.id):
            users.append(u)
    ...
    # only check if flag for gender/identity is "True"
    if str(app.config['MATCHING_FLAG_MENTOR_GENDER_PREFERENCE']) == "True":
        for u in users:  # initialize user dictionary
    ...
    # only check if flag for division preference is "True"
    if str(app.config['MATCHING_FLAG_DIVISION_PREFERENCE']) == "True":
        for u in users:
    ...
    # contains all the matching schools for each user (user : [school])
    schoolDict = {}
    thisUserEducationTagIDs = user.rtn_education()
    thisUserEducationTagIDs = [
        edu.id for edu in thisUserEducationTagIDs]  # get the ids

    for u in users:
        for educ in u.rtn_education():  # cycle thru each user education
            educationTags = EducationTag.query.filter_by(
                educationID=educ.educationID).all()
            # go thru all the educationTags (the ones related to each user ans unique to each input)
            for edTag in educationTags:
                if edTag.id in thisUserEducationTagIDs:
                    if schoolDict.__contains__(u):  # add user to the dict
                        ...
                        # not already in the dict --> add a new array
                        sArr = [edTag.entered_name]
                        schoolDict[u] = sArr
    ...
    # contains all the matching tags for each user (user : [interest tag titles])
    interestTitleDict = {}
    thisUserInterestTagIDs = user.rtn_interests()
    thisUserInterestTagIDs = [
        intrst.id for intrst in thisUserInterestTagIDs]  # get the ids

    for u in users:
        for intrst in u.rtn_interests():  # cycle thru each user education
            interestTags = InterestTag.query.filter_by(
                interestID=intrst.interestID).all()
            # go thru all the educationTags (the ones related to each user ans unique to each input)
    ...
    # contains all the matching career tags for each user (user : [career experience/interest title])
    careerDict = {}
    thisUserCareerInterestIDs = user.rtn_career_interests()
    thisUserCareerInterestIDs = [
        cInt.id for cInt in thisUserCareerInterestIDs]  # get the ids

    for u in users:
        for cInt in u.rtn_career_interests():  # cycle thru each user education
            careerInterestTags = CareerInterestTag.query.filter_by(
                careerInterestID=cInt.careerInterestID).all()
            # go thru all the educationTags (the ones related to each user ans unique to each input)
            for cInt in careerInterestTags:
    ...
    # is now a list of tuples
    sortedDict = sorted(
        userDict.items(), key=lambda item: item[1], reverse=True)
    # sort userDict by value (the number of matches it got in the db.)

    # { user : { match name : [ matches for this user ] } }
    userDictUsefulInfo = {}
...
def feedPost():
    ...
    db.session.commit()
    print("successfully made new selection with",
          User.query.filter_by(id=userMatchID).first())
